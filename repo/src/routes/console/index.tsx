...
function ConsolePage() {
  ...
  const items = [{ label: "Content", icon: FileText }, { label: "Tutor settings", icon: Settings2 }, { label: "Quality flags", icon: ShieldAlert }];
  if (role === "owner") items.unshift({ label: "Overview", icon: LayoutDashboard });
  return <div className="min-h-screen bg-background"><SiteNav /><SidebarProvider><Sidebar collapsible="offcanvas"><SidebarContent><SidebarGroup><SidebarGroupLabel>{role === "owner" ? "Owner console" : "Staff console"}</SidebarGroupLabel><SidebarGroupContent><SidebarMenu>{items.map(({ label, icon: Icon }) => <SidebarMenuItem key={label}><SidebarMenuButton><Icon /><span>{label}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroupContent></SidebarGroup></SidebarContent></Sidebar><SidebarInset><main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6"><header><p className="text-xs font-medium uppercase tracking-[.18em] text-muted-foreground">{role === "owner" ? "Business and content" : "Content operations"}</p><h1 className="mt-1 font-serif text-3xl">{role === "owner" ? "NesAI Nova console" : "Content workspace"}</h1></header>{role === "owner" && metrics && <OwnerMetrics metrics={metrics} />}<section className="mt-8 grid gap-4 md:grid-cols-3"><ConsoleCard icon={Upload} title="Documents" body="Upload, tag, ingest, and review study resources." href="/admin/upload" /><ConsoleCard icon={Settings2} title="Tutors" body="Subject tutor records are stored in Supabase and can be updated without a deploy." /><ConsoleCard icon={ShieldAlert} title="Quality feedback" body="Review flagged answers and close the loop with learners." /></section></main></SidebarInset></SidebarProvider></div>;
}

function ConsoleCard({ icon: Icon, title, body, href }: { icon: LucideIcon; title: string; body: string; href?: string }) { const content=<div className="paper-card h-full p-5"><Icon className="h-5 w-5 text-[var(--color-gold)]"/><h2 className="mt-4 font-serif text-xl">{title}</h2><p className="mt-2 text-sm text-muted-foreground">{body}</p></div>; return href ? <Link to={href as "/admin/upload"}>{content}</Link> : content; }
